    ...
    rules: [
      {
        ...
        matches: [
          `[text*="通知" || text*="公告" || text="广告" || text$="模式" || text$="签到" || text*="是否满意" || text*="好评" || desc*="好评" || text*="评分" || text*="评价" || text*="推送" || text*="免费" || text*="立即" || text*="剪贴板" || text*="开启" || text="去设置" || text*="定位" || text*="位置" || text*="调研" || text*="赞助" || desc*="官网" || text*="交流" || text*="链接" || id$="image"]${CommonPrefix}`,

...
const UpdatePromptId =
  '(id*="close" || id*="Close" || id*="cancel" || id*="Cancel" || id*="update" || id*="Update")&&top>200';

// 通知提示
const InformId =
  '(id*="close" || id$="Close" || id*="cancel" || id*="dialog" || id*="delete" && desc!*="搜索")&&top>200';
    ...
    rules: [
      {
        ...
        // 停止如下页面触发
        excludeMatches:
          '[text^="温馨" || text*="搜索" || text^="猜你" || text="历史记录" || text$="在搜"]',
        action: 'clickCenter',
        anyMatches: [
          `[(text*="跳过" || text*="跳過" || text*="skip" || vid="btn_close") && text.length<10 && width<280]${CommonPrefix}`,
    ...
    rules: [
      {
        key: 0,
        // 停止如下页面触发
        excludeMatches: '[text="关注列表"]',
        matches: [
          '[text*="通知" || text*="权限" || text*="公告" || text="广告" || text$="模式" || text*="喜欢" || text*="SVIP" || text*="好评" || desc*="好评" || text*="评论" || text*="评分" || text*="评价" || text*="获取" || text*="消息" || text*="使用" ||text*="推荐" || text*="发现" || text*="推送" || text*="第一时间" || text*="免费" || desc*="免费" || text*="立即" || text*="剪贴板" || text*="开启" || text="去设置" || text*="使用" || text*="代理" || text*="定位" || text*="位置" || text*="内容" || text*="调研" || text*="分享" || text*="赞助" || text$="提示" || desc$="提示" || desc*="下载" || desc*="官网" || text*="交流" || text*="链接" || text*="关注" || desc*="谢谢" || id$="image"]',
          `[text^="暂不" || text^="暂时" || text$="继续使用" || text$="知道了" || desc$="知道了" || text="取消" || text^="我已知晓" || text*="拒绝" || text^="不再" || text$="再说" || text="不允许" || text*="不了" || text^="下次" || text="考虑一下" || text="没兴趣" || text="關閉" || text="确定" || text="收到" || text="隐藏" || text="否" || ${InformId}]${CommonPrefix}`,
        ],
      },
    ],

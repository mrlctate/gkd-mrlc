    ...
    rules: [
      {
        ...
        // 停止如下页面触发
        excludeMatches:
          '[text^="我已阅读" || text*="退款"][childCount=0][visibleToUser=true]',
        matches: [
          ...
          `[(text^="暂不" || text^="暂时" || text$="继续使用" || text$="知道了" || desc$="知道了" || text="取消"&&top>200 || text="关闭" || text^="我已知晓" || text*="拒绝" || text*="忽略" || text^="不再" || text$="再说" || text="不允许" || text*="不了" || text^="下次" || text="不，谢谢" || text="考虑一下" || text="没兴趣" || text="我没空" || text="關閉" || text="确定" || text="收到" || text="隐藏" || text="否" || text="返回" || text="同意"&&clickable=true&&width<500)&&${CommonIdPostfix}&&text.length<6]${CommonPrefix}`,
        ],
      },
    ],
